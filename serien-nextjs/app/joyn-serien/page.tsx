/**
 * Joyn Serien Hub - Pillar Page for Topical Authority
 * URL: /joyn-serien
 * 
 * Comprehensive hub for all Joyn series including:
 * - Trending series
 * - New releases
 * - Latest news/reviews
 * - All Joyn series catalog
 */
...
export const metadata: Metadata = {
  title: 'Joyn Serien 2026: Neue Releases, Trends & Reviews | serien.de',
  description: 'Entdecke alle Joyn Serien 2026: Neue Releases, Trending Shows, Reviews und News. Dein vollständiger Guide zu den besten Joyn Originals und Exklusiv-Serien.',
  keywords: ['Joyn Serien', 'Joyn 2026', 'Joyn Neue Serien', 'Joyn Originals', 'Joyn Deutschland', 'Streaming Serien'],
  openGraph: {
    title: 'Joyn Serien 2026: Neue Releases, Trends & Reviews',
    description: 'Dein vollständiger Guide zu allen Joyn Serien - Neue Releases, Trends, Reviews und exklusive News.',
    type: 'website',
    url: '/joyn-serien',
  },
  alternates: {
...
const getJoynData = unstable_cache(
  async () => {
    const { resolveStreamerHubTmdbIds } = await import('@/lib/streamer-hub-resolver');
    const tmdbIds = await resolveStreamerHubTmdbIds({
      networks: ["Joyn","ProSieben","SAT.1","Kabel Eins","sixx","ProSieben MAXX","SAT.1 Gold"],
      providers: ["Joyn","Joyn Plus","ProSieben","SAT.1"],
    });
  ...
  ['joyn-hub-data'],
  { revalidate: 600, tags: ['joyn-hub'] }
);

// Schema.org structured data
function generateJoynHubSchema() {
  return {
    ...
    breadcrumb: {
      ...
      itemListElement: [
        ...
        },
        {
          ...
          position: 2,
          name: 'Joyn Serien',
          item: 'https://serien.de/joyn-serien'
        }
      ]
    }
  };
...
export default async function JoynSerienPage() {
  const { allJoynSeries, joynArticles, trendingArticles, recentSeries } = await getJoynData();

  const hubSchema = generateJoynHubSchema();
  ...
  const topRatedSeries = [...allJoynSeries]
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-gray-50 dark:bg-[hsl(230,25%,7%)]">
        {/* Hero Section */}
        <section className="relative bg-gradient-to-br from-pink-500 via-purple-600 to-purple-900 dark:from-red-900 dark:via-red-950 dark:to-black overflow-hidden">
          ...
          <div className="relative max-w-7xl mx-auto px-4 py-16 sm:py-24">
            <div className="text-center">
              ...
              <p className="text-lg sm:text-xl text-white/80 max-w-2xl mx-auto mb-8">
                Dein vollständiger Guide zu allen Joyn Originals und Exklusiv-Serien. 
              ...
              <div className="flex flex-wrap justify-center gap-8 mt-10">
                <div className="text-center">
                  <div className="text-3xl sm:text-4xl font-bold text-white">{allJoynSeries.length}+</div>
                  <div className="text-sm text-white/60 mt-1">Serien</div>
                </div>
                <div className="text-center">
                  <div className="text-3xl sm:text-4xl font-bold text-white">{joynArticles.length}</div>
        ...
        <div className="max-w-7xl mx-auto px-4 py-12 space-y-16">
          ...
          {trendingArticles.length > 0 && (
            <section>
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Trending auf Joyn</h2>
              </div>
                ...
                {trendingArticles.map((article, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Rank Badge */}
                      <div className="absolute top-2 left-2 w-8 h-8 bg-pink-500 rounded-full flex items-center justify-center font-bold text-white text-sm shadow-lg">
                        {index + 1}
                      </div>
                      
                      <div className="absolute bottom-0 left-0 right-0 p-3">
                        <p className="text-white font-semibold text-sm line-clamp-2 group-hover:text-pink-400 transition-colors">
                          {article.title}
                        </p>
                      </div>
                    </Link>
            ...
            <section>
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Neue Joyn Serien</h2>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 sm:gap-6">
                {recentSeries.map((series) => (
                  ...
                  >
                    ...
                    <div className="mt-3">
                      <h3 className="font-semibold text-gray-900 dark:text-white line-clamp-1 group-hover:text-pink-600 dark:group-hover:text-pink-400 transition-colors">
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {joynArticles.slice(0, 6).map((article) => {
                ...
                return (
                  ...
                  >
                    <div className="relative aspect-video overflow-hidden">
                      ...
                      {/* Joyn Badge */}
                      <div className="absolute top-3 right-3 bg-pink-500 text-white text-xs font-bold px-2 py-1 rounded shadow">
                        Joyn
                      </div>
                    </div>
                    
                    <div className="p-4">
                      <h3 className="font-bold text-gray-900 dark:text-white line-clamp-2 group-hover:text-pink-600 dark:group-hover:text-pink-400 transition-colors mb-2">
                        {article.title}
                      </h3>
            ...
            {joynArticles.length > 6 && (
              <div className="text-center mt-8">
                <Link
                  href="/?filter=Joyn"
                  className="inline-flex items-center gap-2 bg-pink-500 hover:bg-red-700 text-white font-semibold px-6 py-3 rounded-lg transition-colors"
                >
                  Alle Joyn News anzeigen
          ...
          {topRatedSeries.length > 0 && (
            <section>
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Top Bewertete Joyn Serien</h2>
              </div>
                ...
                {topRatedSeries.map((series, index) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-bold text-gray-900 dark:text-white group-hover:text-pink-600 dark:group-hover:text-pink-400 transition-colors line-clamp-1">
                        {series.name || series.title}
                      </h3>
          ...
          <section>
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-red-100 dark:bg-pink-400/20 rounded-lg">
                  <Play className="w-5 h-5 text-pink-600 dark:text-pink-400" />
                </div>
                <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Alle Joyn Serien</h2>
              </div>
              <span className="text-sm text-gray-500 dark:text-gray-400">
                {allJoynSeries.length} Serien
              </span>
            </div>
            
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-8 gap-3 sm:gap-4">
              {allJoynSeries.map((series) => (
                <Link
                  key={series.tmdbId}
                  href={`/serie/${series.slug}`}
                  className="group"
                >
                  ...
                  <p className="mt-2 text-xs sm:text-sm font-medium text-gray-900 dark:text-white line-clamp-2 group-hover:text-pink-600 dark:group-hover:text-pink-400 transition-colors">
                    {series.name || series.title}
                  </p>
                </Link>
              ))}
          ...
          <section className="bg-white dark:bg-[hsl(230,25%,10%)] rounded-2xl p-8 shadow-lg border border-gray-200 dark:border-gray-800">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">
              Joyn Serien: Alles was du wissen musst
            </h2>
            
            <div className="prose prose-gray dark:prose-invert max-w-none">
              <p>
                Joyn hat sich seit seiner Gründung zu einem der führenden Streaming-Dienste weltweit entwickelt. 
                Mit einer beeindruckenden Bibliothek von <strong>Joyn Originals</strong> und exklusiven Lizenzserien 
                bietet die Plattform für jeden Geschmack das passende Format.
              </p>
              
              <h3>Was macht Joyn Serien besonders?</h3>
              <p>
                Joyn investiert jährlich Milliarden in die Produktion eigener Inhalte. Von preisgekrönten Dramen 
                wie <em>Stranger Things</em> und <em>The Crown</em> bis hin zu internationalen Hits wie 
                <em>Squid Game</em> und <em>Dark</em> – Joyn Originals setzen regelmäßig neue Maßstäbe 
                in der Serienlandschaft.
              </p>
              
              <h3>Joyn in Deutschland</h3>
              <p>
                Der deutsche Markt ist für Joyn besonders wichtig. Neben internationalen Produktionen 
                finden sich immer mehr deutsche Joyn Originals wie <em>Dark</em>, <em>How to Sell Drugs Online (Fast)</em> 
                und <em>Barbaren</em> im Programm. Diese Serien haben nicht nur in Deutschland, 
                sondern weltweit Erfolge gefeiert.
              </p>
              
              <h3>Neue Joyn Serien 2026</h3>
              <p>
                Auch 2026 erwarten uns zahlreiche neue Joyn Serien. Von lang erwarteten Fortsetzungen 
                bis hin zu völlig neuen Formaten – auf dieser Seite findest du alle aktuellen News, 
                Release-Termine und Reviews zu den neuesten Joyn Produktionen.
              </p>
            </div>
          </section>
