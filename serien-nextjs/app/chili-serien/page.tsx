/**
 * CHILI Serien Hub - Pillar Page for Topical Authority
 * URL: /chili-serien
 * 
 * Comprehensive hub for all CHILI series including:
 * - Trending series
 * - New releases
 * - Latest news/reviews
 * - All CHILI series catalog
 */
...
export const metadata: Metadata = {
  title: 'CHILI Serien 2026: Neue Releases, Trends & Reviews | serien.de',
  description: 'Entdecke alle CHILI Serien 2026: Neue Releases, Trending Shows, Reviews und News. Dein vollständiger Guide zu den besten CHILI Originals und Exklusiv-Serien.',
  keywords: ['CHILI Serien', 'CHILI 2026', 'CHILI Neue Serien', 'CHILI Originals', 'CHILI Deutschland', 'Streaming Serien'],
  openGraph: {
    title: 'CHILI Serien 2026: Neue Releases, Trends & Reviews',
    description: 'Dein vollständiger Guide zu allen CHILI Serien - Neue Releases, Trends, Reviews und exklusive News.',
    type: 'website',
    url: '/chili-serien',
  },
  alternates: {
...
const getCHILIData = unstable_cache(
  async () => {
    ...
    const tmdbIds = await resolveStreamerHubTmdbIds({
      networks: ["CHILI","Chili"],
      providers: ["CHILI","Chili"],
    });
  ...
  ['chili-hub-data'],
  { revalidate: 600, tags: ['chili-hub'] }
);

// Schema.org structured data
function generateCHILIHubSchema() {
  return {
    '@context': 'https://schema.org',
    '@type': 'CollectionPage',
    name: 'CHILI Serien 2026',
    description: 'Vollständiger Guide zu allen CHILI Serien - Neue Releases, Trends und Reviews',
    url: 'https://serien.de/chili-serien',
    ...
    breadcrumb: {
      ...
      itemListElement: [
        ...
        },
        {
          ...
          position: 2,
          name: 'CHILI Serien',
          item: 'https://serien.de/chili-serien'
        }
      ]
    }
  };
...
export default async function CHILISerienPage() {
  const { allCHILISeries, chiliArticles, trendingArticles, recentSeries } = await getCHILIData();

  const hubSchema = generateCHILIHubSchema();
  ...
  const topRatedSeries = [...allCHILISeries]
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-gray-50 dark:bg-[hsl(230,25%,7%)]">
        {/* Hero Section */}
        <section className="relative bg-gradient-to-br from-red-500 via-red-700 to-red-950 dark:from-red-900 dark:via-red-950 dark:to-black overflow-hidden">
          ...
          <div className="relative max-w-[1000px] mx-auto px-4 py-16 sm:py-24">
            <div className="text-center">
              ...
              <p className="text-lg sm:text-xl text-white/80 max-w-2xl mx-auto mb-8">
                Dein vollständiger Guide zu allen CHILI Originals und Exklusiv-Serien. 
              ...
              <div className="flex flex-wrap justify-center gap-8 mt-10">
                <div className="text-center">
                  <div className="text-3xl sm:text-4xl font-bold text-white">{allCHILISeries.length}+</div>
                  <div className="text-sm text-white/60 mt-1">Serien</div>
                </div>
                <div className="text-center">
                  <div className="text-3xl sm:text-4xl font-bold text-white">{chiliArticles.length}</div>
        ...
        <div className="max-w-[1000px] mx-auto px-4 py-12 space-y-16">
          ...
          {trendingArticles.length > 0 && (
            <section>
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Trending auf CHILI</h2>
              </div>
            ...
            <section>
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Neue CHILI Serien</h2>
                </div>
              </div>
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {chiliArticles.slice(0, 6).map((article) => {
          ...
          {topRatedSeries.length > 0 && (
            <section>
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Top Bewertete CHILI Serien</h2>
              </div>
          ...
          <section>
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Alle CHILI Serien</h2>
              </div>
              <span className="text-sm text-gray-500 dark:text-gray-400">
                {allCHILISeries.length} Serien
              </span>
            </div>
            
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-8 gap-3 sm:gap-4">
              {allCHILISeries.map((series) => (
                <Link
                  key={series.tmdbId}
                  href={`/serie/${series.slug}`}
                  className="group"
          ...
          <section className="bg-white dark:bg-[hsl(230,25%,10%)] rounded-2xl p-8 shadow-lg border border-gray-200 dark:border-gray-800">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">
              CHILI Serien: Alles was du wissen musst
            </h2>
            
            <div className="prose prose-gray dark:prose-invert max-w-none">
              <p>
                CHILI hat sich seit seiner Gründung zu einem der führenden Streaming-Dienste weltweit entwickelt. 
                Mit einer beeindruckenden Bibliothek von <strong>CHILI Originals</strong> und exklusiven Lizenzserien 
                bietet die Plattform für jeden Geschmack das passende Format.
              </p>
              
              <h3>Was macht CHILI Serien besonders?</h3>
              <p>
                CHILI investiert jährlich Milliarden in die Produktion eigener Inhalte. Von preisgekrönten Dramen 
                wie <em>Stranger Things</em> und <em>The Crown</em> bis hin zu internationalen Hits wie 
                <em>Squid Game</em> und <em>Dark</em> – CHILI Originals setzen regelmäßig neue Maßstäbe 
                in der Serienlandschaft.
              </p>
              
              <h3>CHILI in Deutschland</h3>
              <p>
                Der deutsche Markt ist für CHILI besonders wichtig. Neben internationalen Produktionen 
                finden sich immer mehr deutsche CHILI Originals wie <em>Dark</em>, <em>How to Sell Drugs Online (Fast)</em> 
                und <em>Barbaren</em> im Programm. Diese Serien haben nicht nur in Deutschland, 
                sondern weltweit Erfolge gefeiert.
              </p>
              
              <h3>Neue CHILI Serien 2026</h3>
              <p>
                Auch 2026 erwarten uns zahlreiche neue CHILI Serien. Von lang erwarteten Fortsetzungen 
                bis hin zu völlig neuen Formaten – auf dieser Seite findest du alle aktuellen News, 
                Release-Termine und Reviews zu den neuesten CHILI Produktionen.
              </p>
            </div>
          </section>
