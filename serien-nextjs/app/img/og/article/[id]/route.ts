/**
 * Article OG Image API - Serves OpenGraph images for articles from Emergent Object Storage
 * Used for social media sharing (Facebook, Twitter, LinkedIn, etc.)
 */
...
export async function GET(request: NextRequest, context: RouteParams) {
  ...
  try {
    ...
    if (!article || !article.heroImagePath) {
      console.warn(`Article ${id} not found or has no heroImagePath for OG image`);
      return NextResponse.redirect(new URL('/og-image.png', request.url));
    }
    ...
    if (!response.ok) {
      console.error(`Failed to fetch OG image from storage: ${storagePath}`);
      return NextResponse.redirect(new URL('/og-image.png', request.url));
    }
