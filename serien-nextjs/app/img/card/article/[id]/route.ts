/**
 * Article Card Image API - Serves card images for articles from Emergent Object Storage
 */
...
export async function GET(request: NextRequest, context: RouteParams) {
  ...
  try {
    // Fetch article from database to get heroImagePath (we use same image for card)
    ...
    if (!article || !article.heroImagePath) {
      console.warn(`Article ${id} not found or has no heroImagePath for card`);
    ...
    if (!response.ok) {
      console.error(`Failed to fetch card image from storage: ${storagePath}`);
      return NextResponse.redirect(new URL('/placeholders/card.webp', request.url));
    }
