/**
 * Paramount+ Serien Hub - Pillar Page for Topical Authority
 * URL: /paramount-plus-serien
 * 
 * Comprehensive hub for all Paramount+ series including:
 * - Trending series
 * - New releases
 * - Latest news/reviews
 * - All Paramount+ series catalog
 */
...
export const metadata: Metadata = {
  title: 'Paramount+ Serien 2026: Neue Releases, Trends & Reviews | serien.de',
  description: 'Entdecke alle Paramount+ Serien 2026: Neue Releases, Trending Shows, Reviews und News. Dein vollständiger Guide zu den besten Paramount+ Originals und Exklusiv-Serien.',
  keywords: ['Paramount+ Serien', 'Paramount+ 2026', 'Paramount+ Neue Serien', 'Paramount+ Originals', 'Paramount+ Deutschland', 'Streaming Serien'],
  openGraph: {
    title: 'Paramount+ Serien 2026: Neue Releases, Trends & Reviews',
    description: 'Dein vollständiger Guide zu allen Paramount+ Serien - Neue Releases, Trends, Reviews und exklusive News.',
    type: 'website',
    url: '/paramount-plus-serien',
  },
  alternates: {
...
const getParamountPlusData = unstable_cache(
  async () => {
    const { resolveStreamerHubTmdbIds } = await import('@/lib/streamer-hub-resolver');
    const tmdbIds = await resolveStreamerHubTmdbIds({
      networks: ["Paramount+","Paramount Plus","Paramount Network","CBS","Showtime"],
      providers: ["Paramount+","Paramount Plus","Paramount"],
    });

    const [
      allParamountPlusSeries,
      paramountPlusArticles,
      trendingArticles,
      recentSeries
    ] = await Promise.all([
      // All Paramount+ series
      ...
      // Recently added Paramount+ series
      prisma.series.findMany({
        where: {
          tmdbId: { in: tmdbIds },
          firstAirDate: {
  ...
  ['paramount-plus-hub-data'],
  { revalidate: 600, tags: ['paramount-plus-hub'] }
);

// Schema.org structured data
function generateParamountPlusHubSchema() {
  return {
    ...
    breadcrumb: {
      ...
      itemListElement: [
        ...
        },
        {
          '@type': 'ListItem',
          position: 2,
          name: 'Paramount+ Serien',
          item: 'https://serien.de/paramount-plus-serien'
        }
      ]
    }
  };
...
export default async function ParamountPlusSerienPage() {
  const { allParamountPlusSeries, paramountPlusArticles, trendingArticles, recentSeries } = await getParamountPlusData();

  const hubSchema = generateParamountPlusHubSchema();
  ...
  const topRatedSeries = [...allParamountPlusSeries]
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-gray-50 dark:bg-[hsl(230,25%,7%)]">
        {/* Hero Section */}
        <section className="relative bg-gradient-to-br from-blue-600 via-blue-800 to-blue-950 dark:from-red-900 dark:via-red-950 dark:to-black overflow-hidden">
          ...
          <div className="relative max-w-[1000px] mx-auto px-4 py-16 sm:py-24">
            <div className="text-center">
              ...
              <p className="text-lg sm:text-xl text-white/80 max-w-2xl mx-auto mb-8">
                Dein vollständiger Guide zu allen Paramount+ Originals und Exklusiv-Serien. 
              ...
              <div className="flex flex-wrap justify-center gap-8 mt-10">
                <div className="text-center">
                  <div className="text-3xl sm:text-4xl font-bold text-white">{allParamountPlusSeries.length}+</div>
                  <div className="text-sm text-white/60 mt-1">Serien</div>
                </div>
                <div className="text-center">
                  <div className="text-3xl sm:text-4xl font-bold text-white">{paramountPlusArticles.length}</div>
        ...
        <div className="max-w-[1000px] mx-auto px-4 py-12 space-y-16">
          ...
          {trendingArticles.length > 0 && (
            <section>
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Trending auf Paramount+</h2>
              </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="absolute bottom-0 left-0 right-0 p-3">
                        <p className="text-white font-semibold text-sm line-clamp-2 group-hover:text-blue-400 transition-colors">
                          {article.title}
                        </p>
                      </div>
                    </Link>
            ...
            <section>
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Neue Paramount+ Serien</h2>
                </div>
              </div>
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {paramountPlusArticles.slice(0, 6).map((article) => {
            ...
            {paramountPlusArticles.length > 6 && (
              <div className="text-center mt-8">
                <Link
                  href="/?filter=Paramount+"
                  className="inline-flex items-center gap-2 bg-blue-600 hover:bg-red-700 text-white font-semibold px-6 py-3 rounded-lg transition-colors"
                >
                  Alle Paramount+ News anzeigen
          ...
          {topRatedSeries.length > 0 && (
            <section>
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Top Bewertete Paramount+ Serien</h2>
              </div>
          ...
          <section>
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-red-100 dark:bg-blue-500/20 rounded-lg">
                  <Play className="w-5 h-5 text-blue-600 dark:text-blue-400" />
                </div>
                <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Alle Paramount+ Serien</h2>
              </div>
              <span className="text-sm text-gray-500 dark:text-gray-400">
                {allParamountPlusSeries.length} Serien
              </span>
            </div>
            
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-8 gap-3 sm:gap-4">
              {allParamountPlusSeries.map((series) => (
                <Link
                  key={series.tmdbId}
                  href={`/serie/${series.slug}`}
                  className="group"
          ...
          <section className="bg-white dark:bg-[hsl(230,25%,10%)] rounded-2xl p-8 shadow-lg border border-gray-200 dark:border-gray-800">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">
              Paramount+ Serien: Alles was du wissen musst
            </h2>
            
            <div className="prose prose-gray dark:prose-invert max-w-none">
              <p>
                Paramount+ hat sich seit seiner Gründung zu einem der führenden Streaming-Dienste weltweit entwickelt. 
                Mit einer beeindruckenden Bibliothek von <strong>Paramount+ Originals</strong> und exklusiven Lizenzserien 
                bietet die Plattform für jeden Geschmack das passende Format.
              </p>
              
              <h3>Was macht Paramount+ Serien besonders?</h3>
              <p>
                Paramount+ investiert jährlich Milliarden in die Produktion eigener Inhalte. Von preisgekrönten Dramen 
                wie <em>Stranger Things</em> und <em>The Crown</em> bis hin zu internationalen Hits wie 
                <em>Squid Game</em> und <em>Dark</em> – Paramount+ Originals setzen regelmäßig neue Maßstäbe 
                in der Serienlandschaft.
              </p>
              
              <h3>Paramount+ in Deutschland</h3>
              <p>
                Der deutsche Markt ist für Paramount+ besonders wichtig. Neben internationalen Produktionen 
                finden sich immer mehr deutsche Paramount+ Originals wie <em>Dark</em>, <em>How to Sell Drugs Online (Fast)</em> 
                und <em>Barbaren</em> im Programm. Diese Serien haben nicht nur in Deutschland, 
                sondern weltweit Erfolge gefeiert.
              </p>
              
              <h3>Neue Paramount+ Serien 2026</h3>
              <p>
                Auch 2026 erwarten uns zahlreiche neue Paramount+ Serien. Von lang erwarteten Fortsetzungen 
                bis hin zu völlig neuen Formaten – auf dieser Seite findest du alle aktuellen News, 
                Release-Termine und Reviews zu den neuesten Paramount+ Produktionen.
              </p>
            </div>
          </section>
