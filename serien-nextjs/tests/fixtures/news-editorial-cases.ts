...
export type EditorialFailureCategory =
  | 'facts' | 'territory' | 'uncertainty' | 'event-status' | 'contradiction'
...
export interface EditorialFixture {
  ...
  sourceText: string;
  expectedOutcome: 'publish' | 'publish-or-hold';
  humanChecks: string[];
...
export const EDITORIAL_FIXTURE_VERSION = 'synthetic-news-v1';
...
export const EDITORIAL_FIXTURES: EditorialFixture[] = [
  ...
  },
  {
    ...
    expectedOutcome: 'publish',
    humanChecks: ['12. Oktober nur USA/Beacon; 20. Oktober nur Benelux/Fenêtre, kein Deutschlandtermin.', 'Wenn Titel oder Vorspann einen Start ankündigt, muss dessen Region dort selbst eindeutig sein.'],
  ...
  },
  {
    ...
    sourceText: `Fictional trade outlet Screen Ledger reports that actor Mara Klee is in negotiations for a role in the planned mystery series Testarchiv. The report cites two people familiar with the negotiations; neither person is identified by name. No contract has been signed, and neither the production company Archivwerk nor Klee's representatives has confirmed her casting. The proposed role is a municipal archivist who discovers a gap in a collection of court records. The project was commissioned for eight episodes in July 2026, before these negotiations began. Writers Ilona Berg and Tom Ahr are developing the scripts, with Berg also serving as showrunner. A production schedule has not been announced. The report says the discussions are ongoing and could still end without an agreement. There is no announced release date or German distribution partner. Screen Ledger does not quote Klee and does not claim to have interviewed her. Other roles have not been announced in this report. The new development is the reported negotiation, not a completed casting decision or a start of filming.`,
    expectedOutcome: 'publish',
...
export const EDITORIAL_HUMAN_RUBRIC = {
  ...
  acceptance: 'Nur abnehmen, wenn alle zutreffenden Mindestwerte erfüllt sind, keine unbelegte Behauptung oder Regions-/Statusverwechslung vorliegt und jede Gegenprobe erkannt wurde. Ein berechtigtes Zurückhalten ist beim Widerspruchsfall zulässig. Sonst Befund mit Textstelle und konkretem Verbesserungsbedarf notieren. Kein Durchschnittswert kann einen Faktenfehler ausgleichen.',
  limitation: 'Sechs synthetische Fälle sind Regressionen, kein Nachweis für die gesamte reale Quellenverteilung. Vor Automatik zusätzlich echte, rechtmäßig nutzbare Quellenfälle und wiederholte Läufe menschlich bewerten.',
};
